...
// in Renderer.swift. Unlike `StampUniforms` there is no world box and no depth
// plane: the shell raymarch samples a 2D coverage map indexed by *direction*, so
// the stamp only needs the paint-time camera basis to project a sky direction
// back to the painted canvas.
...
// space — the same screen→canvas projection as `stamp_density_volume`, minus the
// world box and the depth Gaussian (coverage is a 2D field, not a volume).
//
...
// the Renderer re-dispatches only new dabs, like the volume stamp.
